    ...
    */
    public function dataTable($query)
    {
        ...
        ->addColumn('action', function($delivery){
            $edit_url = route('outbound-delivery.edit', $delivery->id);
            $delete_url = route('outbound-delivery.destroy', $delivery->id);
            if (user_info()->hasAccess('admin.company') || (user_info()->hasAccess('outbound-delivery.update') && user_info()->hasAccess('outbound-delivery.destroy')) ) {
                return view('partials.action-button')->with(compact('edit_url', 'delete_url'));
            } elseif (user_info()->hasAnyAccess(['admin.company', 'outbound-delivery.update'])) {
                return view('partials.action-button')->with(compact('edit_url'));
            } elseif (user_info()->hasAnyAccess(['admin.company', 'outbound-delivery.destroy'])) {

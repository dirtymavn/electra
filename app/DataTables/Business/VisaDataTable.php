...
class VisaDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()->of($query)
            ->addColumn('action', function($type){
                $edit_url = route('businessvisa.edit', $type->id);
                $delete_url = route('businessvisa.destroy', $type->id);
                if (user_info()->hasAccess('admin.company') || (user_info()->hasAccess('businessvisa.update') && user_info()->hasAccess('businessvisa.destroy')) ) {
                    return view('partials.action-button')->with(compact('edit_url', 'delete_url'));
                } elseif (user_info()->hasAnyAccess(['admin.company', 'businessvisa.update'])) {
                    return view('partials.action-button')->with(compact('edit_url'));
                } elseif (user_info()->hasAnyAccess(['admin.company', 'businessvisa.destroy'])) {
    ...
    {
        return 'Business/Visa_' . date('YmdHis');
    }
}

...
@section('page_title', 'LG Lists')

@section('content')
@include('flash::message')
@if(user_info()->hasAnyAccess(['admin.company', 'lg.create']))
    <a href="{{ route('lg.create')}}" class="btn btn-primary" id="btn-submit">
        <i class="fa fa-plus m-right-10"></i> Add LG
    </a>
@endif
@if(user_info()->hasAnyAccess(['admin.company', 'lg.destroy']))
...
<script>
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('lg.bulk-delete') }}", ids);
        } else {

...
@section('page_title', 'Queue Lists')

@section('content')
@include('flash::message')
@if(user_info()->hasAnyAccess(['admin.company', 'businessqueue.create']))
    <a href="{{ route('businessqueue.create')}}" class="btn btn-primary" id="btn-submit">
        <i class="fa fa-plus m-right-10"></i> Add Queue
    </a>
@endif
@if(user_info()->hasAnyAccess(['admin.company', 'businessqueue.destroy']))
...
<script>
    $(document).ready(function() {
        spinnerLoad($('#form-businessqueue'));
    });
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('businessqueue.bulk-delete') }}", ids);
        } else {

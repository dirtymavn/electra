...
<ul class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="{{route('fit-delivery.index')}}">Delivery</a></li>
</ul>
@endsection
...
<div class="row">
    <div class="col-sm-2">
        @if(user_info()->hasAnyAccess(['admin.company', 'fit-delivery.create']))
            <a href="{{ route('fit-delivery.create')}}" class="btn btn-primary" id="btn-submit">
                <i class="fa fa-plus m-right-10"></i> Add Delivery
            </a>
        @endif
    </div>
    <div class="col-sm-2">
        @if(user_info()->hasAnyAccess(['admin.company', 'fit-delivery.destroy']))
    ...
    <div class="col-sm-3">
        <div class="dropdown">
            ...
            <div class="dropdown-menu">
                <a class="dropdown-item" href="{{ route('export.fit-delivery.excel') }}">Export XLS</a>
                <a class="dropdown-item" href="{{ route('export.fit-delivery.pdf') }}">Export PDF</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('fit-delivery.bulk-delete') }}", ids);
        } else {

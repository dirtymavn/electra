...
<ul class="breadcrumb">
    ...
    <li class="breadcrumb-item">Do Type</li>
</ul>
@endsection

@section('page_title', 'Do Type Lists')

@section('content')
@include('flash::message')
@if(user_info()->hasAnyAccess(['admin.company', 'dotype.create']))
    <a href="{{ route('dotype.create')}}" class="btn btn-primary" id="btn-submit">
        <i class="fa fa-plus m-right-10"></i> Add Do Type
    </a>
@endif
@if(user_info()->hasAnyAccess(['admin.company', 'dotype.delete']))
...
<script>
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('dotype.bulk-delete') }}", ids);
        } else {

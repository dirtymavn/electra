...
@section('page_title', 'Branch Lists')

@section('content')
@include('flash::message')
@if(user_info()->hasAnyAccess(['admin.company', 'branch.create']))
    <a href="{{ route('branch.create')}}" class="btn btn-primary" id="btn-submit">
        <i class="fa fa-plus m-right-10"></i> Add Branch
    </a>
@endif
@if(user_info()->hasAnyAccess(['admin.company', 'branch.destroy']))
...
<script>
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('branch.bulk-delete') }}", ids);
        } else {

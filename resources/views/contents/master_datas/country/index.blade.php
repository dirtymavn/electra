...
@section('page_title', 'Country Lists')

@section('content')
@include('flash::message')
@if(user_info()->hasAnyAccess(['admin.company', 'country.create']))
    <a href="{{ route('country.create')}}" class="btn btn-primary" id="btn-submit">
        <i class="fa fa-plus m-right-10"></i> Add Country
    </a>
@endif
@if(user_info()->hasAnyAccess(['admin.company', 'country.delete']))
...
<script>
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('country.bulk-delete') }}", ids);
        } else {

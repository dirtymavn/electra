...
@section('page_title', 'Airline Lists')

@section('content')
@include('flash::message')
<div class="row">
    <div class="col-sm-2">
        @if(user_info()->hasAnyAccess(['admin.company', 'airline.create']))
            <a href="{{ route('airline.create')}}" class="btn btn-primary" id="btn-submit">
                <i class="fa fa-plus m-right-10"></i> Add Airline
            </a>
        @endif
    </div>
    <div class="col-sm-2">
        @if(user_info()->hasAnyAccess(['admin.company', 'airline.destroy']))
    ...
    <div class="col-sm-3">
        <div class="dropdown">
            ...
            <div class="dropdown-menu">
                <a class="dropdown-item" href="{{ route('export.airline.excel') }}">Export XLS</a>
                <a class="dropdown-item" href="{{ route('export.airline.pdf') }}">Export PDF</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('airline.bulk-delete') }}", ids);
        } else {

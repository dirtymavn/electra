...
<ul class="breadcrumb">
    ...
    <li class="breadcrumb-item">Core Config</li>
</ul>
@endsection

@section('page_title', 'Core Config Lists')

@section('content')
@include('flash::message')
@if(user_info()->hasAnyAccess(['admin.company', 'core-config.create']))
    <a href="{{ route('core-config.create')}}" class="btn btn-primary" id="btn-submit">
        <i class="fa fa-plus m-right-10"></i> Add Core Config
    </a>
@endif
@if(user_info()->hasAnyAccess(['admin.company', 'core-config.destroy']))
...
<script>
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('core-config.bulk-delete') }}", ids);
        } else {

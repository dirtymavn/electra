...
<ul class="breadcrumb">
    ...
    <li class="breadcrumb-item">Core Status</li>
</ul>
@endsection

@section('page_title', 'Core Status Lists')

@section('content')
@include('flash::message')
@if(user_info()->hasAnyAccess(['admin.company', 'core-status.create']))
    <a href="{{ route('core-status.create')}}" class="btn btn-primary" id="btn-submit">
        <i class="fa fa-plus m-right-10"></i> Add Core Status
    </a>
@endif
@if(user_info()->hasAnyAccess(['admin.company', 'core-status.destroy']))
...
<script>
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('core-status.bulk-delete') }}", ids);
        } else {

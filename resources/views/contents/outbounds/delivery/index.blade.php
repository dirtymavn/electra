...
<div class="row">
    <div class="col-sm-2">
        @if(user_info()->hasAnyAccess(['admin.company', 'outbound-delivery.create']))
            <a href="{{ route('outbound-delivery.create')}}" class="btn btn-primary" id="btn-submit">
                <i class="fa fa-plus m-right-10"></i> Add Delivery
            </a>
        @endif
    </div>
    <div class="col-sm-2">
        @if(user_info()->hasAnyAccess(['admin.company', 'outbound-delivery.destroy']))
    ...
    <div class="col-sm-3">
        <div class="dropdown">
            ...
            <div class="dropdown-menu">
                <a class="dropdown-item" href="{{ route('export.outbound-delivery.excel') }}">Export XLS</a>
                <a class="dropdown-item" href="{{ route('export.outbound-delivery.pdf') }}">Export PDF</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).on('click', '#bulk-delete', function() {
        ...
        if (ids.length > 0) {
            bulkDelete("{{ route('outbound-delivery.bulk-delete') }}", ids);
        } else {
